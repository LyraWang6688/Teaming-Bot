...
- 本地仅编辑代码、执行静态检查和不启动服务的单元测试。禁止启动 `next dev`、`next start`、飞书监听、任务消费者或本地预览服务。
- 所有业务服务只在部署服务器运行；生产容器须显式设置 `FEISHU_RUNTIME_ENABLED=true`。不得在本地设置该开关来绕过限制。
- 修改后在服务器受控验证；不得让本地进程连接生产数据库消费任务。
...
# 本地开发服务已禁用；不要启动 next dev / next start
