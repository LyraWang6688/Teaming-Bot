...
export const feishuProjects = pgTable(
  'feishu_projects',
  {
    ...
    bitableAppTokenEncrypted: text('bitable_app_token_encrypted'),
    bitableTableId: text('bitable_table_id'),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
...
export const meetingRecords = pgTable(
  'meeting_records',
  {
    ...
    reportPublicId: uuid('report_public_id').defaultRandom().notNull(),
  ...
  (table) => [
    ...
    uniqueIndex('meeting_records_report_public_id_uidx').on(table.reportPublicId),
    index('meeting_records_base_record_id_idx').on(table.baseRecordId),
  ]
);
