...
import type { NetworkEdgeV1, NetworkNodeV1, PlayerRoleV1 } from '@/types';
...
const ROLE_CONFIG: Record<PlayerRoleV1, { fill: string; stroke: string; text: string; label: string }> = {
...
function computeStaticLayout(nodes: NetworkNodeV1[], width: number, height: number) {
  const cx = width / 2;
  const cy = height / 2;
  const padding = 60;
  const radius = Math.min(width, height) / 2 - padding;

  const groups: Record<PlayerRoleV1, NetworkNodeV1[]> = {
    mover: [],
    follower: [],
    opposer: [],
    bystander: [],
  ...
  const sectorAngles: Record<PlayerRoleV1, [number, number]> = {
    mover: [-90, -90],
    follower: [-50, 40],
    opposer: [-180, -80],
    bystander: [40, 130],
  ...
  (['follower', 'opposer', 'bystander', 'silent'] as PlayerRoleV1[]).forEach((role) => {
